...
$template['header_link'] = 'VENDAS EFETUADAS';
...
if(isset($dataInicio) && isset($dataFim))
{
	...
	$xml = "<sgsm:GetCertificadosByEmpresa>
	...
	$xml .= "</sgsm:GetCertificadosByEmpresa>";
//var_dump($xml);exit;
	$response = $obj1->GetCertificadosByEmpresaNOSHOW("$xml");
	$response = str_replace("</soap:Body>","",str_replace("<soap:Body>","",$response));
	$response = simplexml_load_string($response);
	if(isset($response->GetCertificadosByEmpresaResponse->GetCertificadosByEmpresaResult->GetCertificadoRetorno)
		&& strval($response->GetCertificadosByEmpresaResponse->GetCertificadosByEmpresaResult->GetCertificadoRetorno->CodigoRetorno)=="0")
	{
		$arrayCertificados = $response->GetCertificadosByEmpresaResponse->GetCertificadosByEmpresaResult->GetCertificadoRetorno->ListaCertificado;
	} else {
		$descricaoErro = utf8_decode(strval($response->GetCertificadosByEmpresaResponse->GetCertificadosByEmpresaResult->GetCertificadoRetorno->MensagemAmigavel));
	}
	unset($obj1);
}
            ...
            <div class="col-sm-6 hidden-xs">
                <div class="header-section">
                    <ul class="breadcrumb breadcrumb-top">
                        <li>Home</li>
                        <li>Controle</li>
                        <li><a href="page_form_vendasefetuadas.php">Vendas Efetuadas</a></li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div class="block-title">
		 ...
		 <!-- Inline Form Block -->
            <div class="block full">
                ...
                <!-- Inline Form Content -->
                <form action="page_form_vendasefetuadas.php" method="post" class="form-inline" >
            ...
            <table id="example-datatable" class="table table-striped table-bordered table-vcenter">
                <thead>
                    <tr>
						...
						<th>Data Adesão</th>
                        <th>CPF Cliente</th>
                        <!--<th style="width: 120px;">Email do Cliente</th>-->
                        <th>Evento</th>
						<th>Data Evento</th>
                        <th class="text-center" style="width: 45px;"><i class="fa fa-flash"></i></th>
                    </tr>
                </thead>
                <tbody>
					<?php
					$labels['0']['class'] = "label-success";
                    $labels['0']['text'] = "Emitida";
                    $labels['1']['class'] = "label-info";
                    $labels['1']['text'] = "On hold..";
                    $labels['2']['class'] = "label-danger";
                    $labels['2']['text'] = "Cancelada";
					...
					if($descricaoErro=='' && $arrayCertificados != null)
					{
						...
						?>
						<tr>
							<td class="text-center"><?php echo $obj->NumeroProposta; ?></td>
							<td><?php echo date("d/m/Y h:i", strtotime($obj->DataAdesao)); ?></td>
							<td><strong><?php echo $obj->Cliente->Contato->CPF; ?></strong></td>
							<!--<td><?php echo utf8_decode($obj->Cliente->Contato->Email); ?></td>-->
							<td><?php echo utf8_decode($obj->Evento->Nome); ?></td>
							<td><?php echo date("d/m/Y", strtotime($obj->Evento->Data)); ?></td>
							<?php $rand = rand(0, 3); ?>
							<!--<td><span class="label<?php echo ($labels[$rand]['class']) ? " " . $labels[$rand]['class'] : ""; ?>"><?php echo $labels[$rand]['text'] ?></span></td>-->
							<td class="text-center">
								<a href="javascript:void(0)" data-toggle="tooltip" title="Baixar certificado" class="btn btn-effect-ripple btn-xs btn-success" certificado="<?php echo $obj->NumeroProposta; ?>" cpf="<?php echo $obj->Cliente->Contato->CPF; ?>"><i class="gi gi-disk_save"></i></a>
								<a href="javascript:void(0)" data-toggle="tooltip" title="Comunicar cliente" class="btn btn-effect-ripple btn-xs btn-success" certificado="<?php echo $obj->NumeroProposta; ?>" cpf="<?php echo $obj->Cliente->Contato->CPF; ?>" email="<?php echo $obj->Cliente->Contato->Email; ?>"><i class="gi gi-send"></i></a>
								<!--
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h3 class="modal-title"><strong>COMUNICAÇÃO DA VENDA</strong></h3>
				</div>
				<div class="modal-body">
					<form id="form-comunica" action="ajax/comunicavenda.php" method="post" class="form-horizontal form-bordered">
						<input type="hidden" id="val-cpf" name="val-cpf" />
						<input type="hidden" id="val-certificado" name="val-certificado" />
						<BR/>
						<h4>COMUNICAÇÃO DE BOAS VINDAS</h4>
						<ul>
							<li>Certificado: <span id="texto_certificado"></span></li>
							<li>CPF: <span id="texto_cpf"></span></li>
						</ul>
								<div class="form-group">
